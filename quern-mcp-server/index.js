#!/usr/bin/env bun
/**
 * Quern MCP server — compile with Bun for Claude Desktop:
 *   bun install && bun build ./index.js --compile --outfile quern-mcp-server
 *
 ...
 * DB: ~/Library/Application Support/Quern/meetings.db
 */
...
const quernDB = path.join(os.homedir(), "Library/Application Support/Quern/meetings.db");
const legacyDB = path.join(os.homedir(), "Library/Application Support/Grist/meetings.db");
const dbPath = fs.existsSync(quernDB)
  ? quernDB
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: "list_folders",
        description: "List all folders in Quern.",
        inputSchema: { type: "object", properties: {} },
      },
      {
        name: "list_notes",
        description:
          "List notes/meetings in Quern (id, title, folder, timestamps). Use before update_note to find an id.",
        inputSchema: {
          type: "object",
          properties: {
            query: {
      ...
      },
      {
        name: "create_note",
        description:
          "Create a new note in Quern (Markdown content). Use update_note to change an existing item.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        name: "update_note",
        description:
          "Update an existing Quern note/meeting. Identify by id (preferred) or title. Can change title, notes body, summary, and/or folder. Set append=true to append to notes/summary instead of replacing.",
        inputSchema: {
          type: "object",
          properties: {
            id: {
      ...
      },
      {
        name: "list_tasks",
        description:
          "List Quern tasks. Filter by status (open/done/all), source meeting, or text query.",
        inputSchema: {
          type: "object",
          properties: {
            status: {
      ...
      {
        name: "delete_task",
        description: "Soft-delete a task (hidden in Quern, not hard-removed).",
        inputSchema: {
          type: "object",
          properties: {
            id: { type: "string" },
...
async function run() {
  ...
  console.error("Quern MCP Server running on stdio");
}
