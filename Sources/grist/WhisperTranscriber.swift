...
class WhisperTranscriber: @unchecked Sendable {
    ...
    func transcribe(meetingId: String) async -> String {
        ...
        // Step 2: Locate Whisper binary and model
        ...
        if whisperModel == nil || whisperModel!.isEmpty || !FileManager.default.fileExists(atPath: whisperModel!) {
            ...
            if !FileManager.default.fileExists(atPath: whisperModel!) {
                whisperModel = "\(whisperDir)/models/ggml-base.bin" // fallback to base
            }
        }
        
        guard let finalModel = whisperModel, FileManager.default.fileExists(atPath: finalModel) else {
            return "[Error: Whisper model not found at \(whisperModel ?? "nil")]"
        }
        ...
        guard let finalBinary = whisperBinary, FileManager.default.fileExists(atPath: finalBinary) else {
            return "[Error: whisper.cpp binary not found at \(whisperBinary ?? "nil")]"
        }
