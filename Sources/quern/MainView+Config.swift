...
extension MainView {
    ...
    func sendToObsidian(meeting: Meeting) {
        do {
            ...
            QuernLog.log("[Obsidian] send ok \(url.path)")
        } catch {
            statusMessage = "Obsidian: \(error.localizedDescription)"
            QuernLog.log("[Obsidian] send failed: \(error.localizedDescription)")
        }
    }
    ...
    var importURLSheet: some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 18) {
                if canAppend {
                    QuernLabeledField(label: "Destination") {
                ...
                QuernLabeledField(label: n > 1 ? "URLs (\(n))" : "URL(s)") {
                ...
                if ytCount > 0 {
                    QuernInfoCard(tint: YouTubeImporter.resolveYtDlpPath() == nil ? .orange : .red) {
                        HStack(spacing: 8) {
                ...
                if !(importAppendToSelected && canAppend) {
                    QuernLabeledField(label: "Folder") {
                        VStack(alignment: .leading, spacing: 10) {
    ...
    func refreshHealthCheck(showSheetIfNeeded: Bool) {
        ...
        Task {
            let report = await QuernHealth.check()
            await MainActor.run {
                healthReport = report
    ...
    @ViewBuilder
    func healthBanner(_ report: QuernHealthReport) -> some View {
    ...
    var healthChecklistSheet: some View {
        VStack(alignment: .leading, spacing: 0) {
            QuernSheetHeader(
                ...
                subtitle: "Quern works best with local Ollama models and optional capture tools.",
                    ...
                    if let report = healthReport {
                        ...
                        if !report.issues.isEmpty {
                            ...
                            ForEach(report.issues) { issue in
                                QuernInfoCard(tint: issue.isBlocking ? .orange : .blue) {
