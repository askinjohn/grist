        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                HStack(alignment: .center, spacing: 12) {
                    QuernIconBadge(systemName: "gearshape.2.fill", tint: .accent, size: 36)
            ...
            VStack(alignment: .leading, spacing: 28) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    settingsToggleRow(
                        ...
                        subtitle: "Watch for Zoom, Teams, Google Meet, or Webex and offer to record in Quern.",
            ...
            VStack(alignment: .leading, spacing: 24) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("AI models by job")
                        .font(.title3.weight(.semibold))
                    Text("Each feature can use a different model and backend (local Ollama or OpenAI-compatible). Edit the table or the raw JSON below. File: Application Support/Quern/ai-config.json")
                        .font(.callout)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    HStack(spacing: 12) {
                        QuernIconBadge(systemName: "book.closed.fill", tint: .purple, size: 40)
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Obsidian")
                                .font(.headline)
                            Text("Write Quern notes as Markdown into your vault")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                        Spacer()
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Subfolder")
                            .font(.callout)
                        TextField("Quern", text: $draft.subfolder)
                            .textFieldStyle(.roundedBorder)
                            .frame(maxWidth: .infinity)
                    }
...
struct VoiceSettingsCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 12) {
                QuernIconBadge(systemName: "speaker.wave.2.fill", tint: .purple, size: 40)
                VStack(alignment: .leading, spacing: 2) {
                    Text("Read aloud")
                        .font(.headline)
            ...
            HStack {
                ...
                Spacer()
                Button("Test voice") {
                    speech.speak("Hello from Quern. This is your read aloud voice.")
                }
                .buttonStyle(.borderedProminent)
                .tint(.purple)
            }
