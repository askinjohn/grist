...
struct QuernIconBadge: View {
    let systemName: String
    var tint: QuernSheetTint = .accent
    var size: CGFloat = 44
...
struct QuernSheetHeader: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 14) {
            QuernIconBadge(systemName: systemImage, tint: tint)
...
struct QuernSheetFooter<Leading: View, Trailing: View>: View {
...
extension QuernSheetFooter where Leading == EmptyView {
...
struct QuernLabeledField<Content: View>: View {
    let label: String
...
struct QuernFieldBackground: ViewModifier {
...
extension View {
    func gristFieldStyle(minHeight: CGFloat? = nil) -> some View {
        modifier(QuernFieldBackground(minHeight: minHeight))
    }
}

/// Card used for empty states and choice rows.
struct QuernInfoCard<Content: View>: View {
    var tint: QuernSheetTint = .accent
    @ViewBuilder var content: () -> Content
