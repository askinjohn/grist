...
final class MeetingPromptController: NSObject, UNUserNotificationCenterDelegate {
    ...
    func configure() {
        ...
        center.requestAuthorization(options: [.alert, .sound, .badge]) { ok, error in
            Task { @MainActor in
                self.authorized = ok
                if let error {
                    QuernLog.log("[MeetingPrompt] auth error: \(error.localizedDescription)")
                } else {
                    QuernLog.log("[MeetingPrompt] notifications authorized=\(ok)")
                }
            }
        }
    }
    ...
    func present(detection: MeetingDetector.Detection) {
        ...
        guard authorized else {
            ...
            QuernLog.log("[MeetingPrompt] no notification permission — menu bar only")
            return
        }

        let content = UNMutableNotificationContent()
        content.title = "Meeting detected"
        content.body = "\(detection.appName) looks like an active call. Record in Quern?"
        ...
        let req = UNNotificationRequest(
            identifier: "quern-meeting-\(detection.appName)-\(Int(Date().timeIntervalSince1970))",
            content: content,
            trigger: nil
        )
        UNUserNotificationCenter.current().add(req) { err in
            if let err {
                QuernLog.log("[MeetingPrompt] add failed: \(err.localizedDescription)")
            }
        }
    }
