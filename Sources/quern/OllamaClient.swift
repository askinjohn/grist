...
class OllamaClient: @unchecked Sendable {
    ...
    private func generateText(
        ...
        QuernLog.log("[OllamaClient] generate role=\(role.rawValue) backend=\(ep.backendName) model=\(useModel) promptChars=\(prompt.count) timeout=\(Int(timeout))s")
        ...
        if ep.isOllama {
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                QuernLog.log("[OllamaClient] generate FAILED no HTTP response after \(String(format: "%.1f", elapsed))s")
            ...
            guard httpResponse.statusCode == 200 else {
                ...
                QuernLog.log("[OllamaClient] generate FAILED HTTP \(httpResponse.statusCode) after \(String(format: "%.1f", elapsed))s model=\(useModel) body=\(body)")
            ...
            QuernLog.log("[OllamaClient] generate OK \(String(format: "%.1f", elapsed))s model=\(useModel) responseChars=\(text.count)")
            return text
        } else {
            ...
            guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                ...
                QuernLog.log("[OllamaClient] generate OpenAI FAILED HTTP \(code) after \(String(format: "%.1f", elapsed))s")
            ...
            let text = try JSONDecoder().decode(OpenAIChatResponse.self, from: data).choices.first?.message.content
                .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            QuernLog.log("[OllamaClient] generate OpenAI OK \(String(format: "%.1f", elapsed))s responseChars=\(text.count)")
            return text
        }
    }
    ...
    static func truncateForEnhance(_ text: String, maxChars: Int, label: String) -> String {
        ...
        QuernLog.log("[OllamaClient] truncate \(label): \(t.count) → \(maxChars) chars (head+mid+tail)")
        return """
        \(head)
    ...
    static func notesForEnhance(notes: String, transcript: String) -> String {
        ...
        if notesDuplicateTranscript {
            ...
            QuernLog.log("[OllamaClient] notes de-duped vs transcript: \(n.count) chars → header \(h.count) chars")
            return h
        }
    ...
    func enhance(
        ...
        if transcript.count > Self.enhanceMapReduceThreshold {
            QuernLog.log("[OllamaClient] enhance map-reduce path (\(transcript.count) chars)")
        ...
        QuernLog.log("[OllamaClient] enhance after prep transcriptChars=\(body.count) notesChars=\(cleanedNotes.count)")
    ...
    private func enhanceSinglePass(
        ...
        QuernLog.log("[OllamaClient] enhance summaryPreview: \(sumPrev.prefix(200))…")
        return parsed
    }
    ...
    func folderSummarize(
        ...
        QuernLog.log("[OllamaClient] Folder summarize '\(folderName)' (\(items.count) items) model=\(model)")
    ...
    func chat(
        ...
        let ep = endpointSync(for: role, modelOverride: model)
        let useModel = ep.model
        QuernLog.log("[OllamaClient] chat role=\(role.rawValue) backend=\(ep.backendName) model=\(useModel)")
    ...
    func getEmbedding(text: String, model: String = "nomic-embed-text") async throws -> [Double] {
        ...
        QuernLog.log("[OllamaClient] embed backend=\(ep.backendName) model=\(useModel)")
